...
@Injectable()
export class LeaveBalanceUserInterceptor implements NestInterceptor {
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    if (user.role === Roles.EMPLOYEE || user.role === Roles.DRIVER) {
      if (request.query.userIds) {
        throw new BadRequestException(LEAVE_BALANCE_ERRORS.EMPLOYEE_CANNOT_SPECIFY_USER_IDS);
      }
